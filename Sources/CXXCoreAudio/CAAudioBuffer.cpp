...
//		throw std::invalid_argument("invalid audio format");
	...
	const auto framesToInsert = std::min(frameCapacity_ - frameLength_, std::min(frameLength, buffer.frameLength_ - readOffset));
	...
	if(framesToInsert) {
		for(UInt32 i = 0; i < buffer.bufferList_->mNumberBuffers; ++i)
			std::memcpy(reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(bufferList_->mBuffers[i].mData) + (writeOffset * format_.mBytesPerFrame)),
						reinterpret_cast<const void *>(reinterpret_cast<uintptr_t>(buffer.bufferList_->mBuffers[i].mData) + (readOffset * format_.mBytesPerFrame)),
...
bool CXXCoreAudio::CAAudioBuffer::adopt(AudioBufferList *bufferList, const AudioStreamBasicDescription& format, UInt32 frameCapacity, UInt32 frameLength) noexcept
{
	if(!bufferList)
		return false;
	if(format.mBytesPerFrame == 0 || format.mChannelsPerFrame == 0)
	...
	if(frameLength > frameCapacity)
		return false;
	if(bufferList->mNumberBuffers != (format.mFormatFlags & kAudioFormatFlagIsNonInterleaved) ? format.mChannelsPerFrame : 1)
		return false;
