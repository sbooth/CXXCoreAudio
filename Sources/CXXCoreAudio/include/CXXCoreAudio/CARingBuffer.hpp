...
class CARingBuffer final {
  public:
    ...
    ///
    /// The format-specific capacity is the largest integral power of two not greater than
    /// std::numeric_limits<uint32_t>::max() / format.mBytesPerFrame.
    ///
    ...
    /// @note The usable ring buffer capacity will be one less than the smallest integral power of two that is not less
