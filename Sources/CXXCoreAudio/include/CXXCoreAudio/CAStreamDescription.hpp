...
//	CAStreamDescription& operator=(CAStreamDescription&&) = delete;
	...
	bool operator==(const AudioStreamBasicDescription& other) const noexcept 	{ return !std::memcmp(this, &other, sizeof(AudioStreamBasicDescription)); }
	/// Returns true if other is not equal to this.
	bool operator!=(const AudioStreamBasicDescription& other) const noexcept 	{ return !operator==(other); }
	...
	bool IsNonInterleaved() const noexcept 				{ return (mFormatFlags & kAudioFormatFlagIsNonInterleaved) == kAudioFormatFlagIsNonInterleaved; }
	/// Returns true if the kAudioFormatFlagIsNonInterleaved flag is clear.
	bool IsInterleaved() const noexcept 				{ return (mFormatFlags & kAudioFormatFlagIsNonInterleaved) == 0; }

	/// Returns the number of interleaved channels.
	UInt32 InterleavedChannelCount() const noexcept 	{ return IsInterleaved() ? mChannelsPerFrame : 1; }
	/// Returns the number of channel streams.
	UInt32 ChannelStreamCount() const noexcept 			{ return IsInterleaved() ? 1 : mChannelsPerFrame; }
	/// Returns the number of channels.
	UInt32 ChannelCount() const noexcept 				{ return mChannelsPerFrame; }

	/// Returns true if mFormatID == kAudioFormatLinearPCM.
	bool IsPCM() const noexcept 						{ return mFormatID == kAudioFormatLinearPCM; }

	/// Returns true if the kAudioFormatFlagIsBigEndian flag is set.
	bool IsBigEndian() const noexcept 					{ return (mFormatFlags & kAudioFormatFlagIsBigEndian) == kAudioFormatFlagIsBigEndian; }
	/// Returns true if the kAudioFormatFlagIsBigEndian flag is clear.
	bool IsLittleEndian() const noexcept 				{ return (mFormatFlags & kAudioFormatFlagIsBigEndian) == 0; }
	/// Returns true if this format is native-endian.
	bool IsNativeEndian() const noexcept 				{ return (mFormatFlags & kAudioFormatFlagIsBigEndian) == kAudioFormatFlagsNativeEndian; }

	/// Returns true if this format is linear PCM and the kAudioFormatFlagIsFloat flag is set.
	bool IsFloat() const noexcept 						{ return IsPCM() && (mFormatFlags & kAudioFormatFlagIsFloat) == kAudioFormatFlagIsFloat; }
	/// Returns true if this format is linear PCM and the kAudioFormatFlagIsFloat flag is clear.
	bool IsInteger() const noexcept 					{ return IsPCM() && (mFormatFlags & kAudioFormatFlagIsFloat) == 0; }
	/// Returns true if this format is linear PCM and the kAudioFormatFlagIsSignedInteger flag is set.
	bool IsSignedInteger() const noexcept 				{ return IsPCM() && (mFormatFlags & kAudioFormatFlagIsSignedInteger) == kAudioFormatFlagIsSignedInteger; }

	/// Returns true if the kAudioFormatFlagIsPacked flag is set.
	bool IsPacked() const noexcept 						{ return (mFormatFlags & kAudioFormatFlagIsPacked) == kAudioFormatFlagIsPacked; }
	...
	bool IsImplicitlyPacked() const noexcept 			{ return ((mBitsPerChannel / 8) * InterleavedChannelCount()) == mBytesPerFrame; }
	/// Returns true if this format is linear PCM and the sample bits do not occupy the entire channel.
	bool IsUnpackedPCM() const noexcept 				{ return IsPCM() && (SampleWordSize() << 3) != mBitsPerChannel; }

	/// Returns true if the kAudioFormatFlagIsAlignedHigh flag is set.
	bool IsAlignedHigh() const noexcept 				{ return (mFormatFlags & kAudioFormatFlagIsAlignedHigh) == kAudioFormatFlagIsAlignedHigh; }
	/// Returns true if this format is unpacked linear PCM or if mBitsPerChannel is not a multiple of 8.
	bool IsUnaligned() const noexcept 					{ return IsUnpackedPCM() || (mBitsPerChannel & 7) != 0; }

	/// Returns the number of fractional bits.
	UInt32 FractionalBits() const noexcept 				{ return (mFormatFlags & kLinearPCMFormatFlagsSampleFractionMask) >> kLinearPCMFormatFlagsSampleFractionShift; }
	/// Returns true if this format is integer fixed-point linear PCM.
	bool IsFixedPoint() const noexcept 					{ return IsInteger() && FractionalBits() > 0; }
	...
	bool IsNonMixable() const noexcept 					{ return (mFormatFlags & kAudioFormatFlagIsNonMixable) == kAudioFormatFlagIsNonMixable; }
	/// Returns true if this format is linear PCM and the kAudioFormatFlagIsNonMixable flag is clear.
	/// @note This flag is only used when interacting with HAL stream formats.
	bool IsMixable() const noexcept 					{ return IsPCM() && (mFormatFlags & kAudioFormatFlagIsNonMixable) == 0; }
